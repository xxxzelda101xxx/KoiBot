...
exports.osutrack = function(osu, bot){
	...
	var filename = __dirname + "/osutracking/"+osu.channel+".json";
	var trackedchannel = require(filename);
	if (osu === null) return;
		...
		if (data) {
			...
			if (parseFloat(osupp) > parseFloat(osu.pp) + 1) { 
				if (parseInt(osu.Rank - osurank) < 0) {
					...
					osu.Rank = osurank;
					osu.pp = osupp;
					require("fs").writeFile(filename,JSON.stringify(allchannels[osu.channel],null,2), null);
					return;
				}
				if (osu.recent != recent && mapid) {
					if (data.events[0].display_html.indexOf("(osu!)") > -1) {
			...
			if (parseFloat(osupp) < parseFloat(osu.pp) - 1) { 
				if (osu.channel === "118689714319392769") {
					if (parseInt(osu.Rank - osurank) < 0) {
						bot.sendMessage(osu.channel, "**"+osu.Username+"** lost **"+parseFloat(osupp - osu.pp).toFixed(2)+"pp** and lost **"+parseInt(osurank - osu.Rank)+" rank(s)**! Current Rank: **"+osurank+"** Current pp: **"+osupp+"** Mode: **osu!**")
						if (osu.recent != recent && mapid) {
							if (data.events[0].display_html.indexOf("(osu!)") > -1) {
								if (data.username != undefined) {
									topplays(osu, data, mapid, mode, bot);
					...
					bot.sendMessage(osu.channel, "**"+osu.Username+"** lost **"+parseFloat(osupp - osu.pp).toFixed(2)+"pp** and gained **"+parseInt(osu.Rank - osurank)+" rank(s)**! Current Rank: **"+osurank+"** Current pp: **"+osupp+"** Mode: **osu!**")
				}
			}
...
function topplays(osu, data, mapid, mode, bot) {
	...
	osu1.getScoresRaw({ b: mapid, m: mode, u: data.user_id, type: "id" }, function(err, data1) {
		...
		osu1.getUserBestRaw({ u: data.user_id, m: mode, limit: 100, type: "id" }, function (err, data2) {
			...
			scoredata(data1, data2, bot, osu)
        });
	})
}

function scoredata(data1, data2, bot, osu) {
	...
	setTimeout(function(){
		bot.sendMessage(osu.channel, "**#"+topscore+"** top play! Rank: **"+rank+"** pp: **"+pp+"** Mods: **"+modds1+"**") 
	}, 200)
}